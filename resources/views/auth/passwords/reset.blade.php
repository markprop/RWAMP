        ...
        <div class="bg-white/95 backdrop-blur rounded-2xl shadow-2xl p-8 card-hover animate-fadeInUp">
            ...
            @if ($errors->any())
                <div class="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded mb-4">
                    ...
                    @if(str_contains($errors->first(), 'invalid') || str_contains($errors->first(), 'expired'))
                        <p class="mt-2 text-sm">The password reset link may have expired (links expire after 60 minutes) or is invalid. Please <a href="{{ route('password.request') }}" class="text-blue-600 underline">request a new password reset link</a>.</p>
                    @endif
                </div>
            @endif
            @if(session('status'))
                ...
                <div>
                    ...
                    <div class="relative">
                        <input 
                            name="password"
                            type="password"
                            x-bind:type="showPassword.primary ? 'text' : 'password'" 
                            class="form-input pr-20" 
                            required 
                            autocomplete="new-password"
                            x-on:keydown="checkCapsLock($event, 'primary')"
                            x-on:keyup="checkCapsLock($event, 'primary')"
                        />
                        <div class="absolute inset-y-0 right-0 pr-3 flex items-center gap-2">
                            <!-- Caps Lock Indicator -->
                            <span x-show="capsLockActive.primary" x-cloak class="text-xs text-amber-600 font-semibold" title="Caps Lock is ON">⇪</span>
                            <!-- Num Lock Indicator -->
                            <span x-show="numLockActive.primary" x-cloak class="text-xs text-blue-600 font-semibold" title="Num Lock is ON">🔢</span>
                            <!-- Password Visibility Toggle (Always visible) -->
                            <button 
                                type="button"
                                @click="showPassword.primary = !showPassword.primary"
                                class="text-gray-500 hover:text-gray-700 focus:outline-none transition-colors"
                                :title="showPassword.primary ? 'Hide password' : 'Show password'"
                                tabindex="-1"
                            >
                                <!-- Eye Icon (Visible) -->
                                <svg x-show="showPassword.primary" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" x-cloak>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                </svg>
                                <!-- Eye Slash Icon (Hidden) -->
                                <svg x-show="!showPassword.primary" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input 
                            ...
                            type="password"
                            x-bind:type="showPassword.confirm ? 'text' : 'password'" 
                            class="form-input pr-20" 
                            required 
                            autocomplete="new-password"
